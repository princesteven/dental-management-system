...
class Finance extends MX_Controller {
    ...
    public function addPayment() {
        ...
        $insurance_type = $this->input->post('insurance_type') ?: null;
        $insurance_number = $this->input->post('insurance_number') ?: null;
        ...
        $is_new = $this->input->post('is_new_patient');
...
// redirect('accountant/add_new'); 
    ...
    public function paymentCategory() {
        ...
        $data['categories'] = $this->finance_model->getPaymentCategory();
        $data['treatments'] = $this->finance_model->getPaymentCategoryTypes();
        $data['settings'] = $this->settings_model->getSettings();
        $this->load->view('home/dashboard'); // just the header file
...
// Validating Description Field
    ...
    function invoice() {
        $id = $this->input->get('id');
        $data['payment'] = $this->finance_model->getPaymentById($id);
        $data['pharmacy_payment'] = $this->pharmacy_model->getPaymentByInvoiceId($id);
    ...
    function patientPaymentHistory() {
        ...
        if (!empty($date_from)) {
            ...
            $data['pharmacy_payments'] = $this->pharmacy_model->getPaymentByPatientIdByDate($patient, $date_from, $date_to);
        } else {
...
// Validating Deposited Amount Field
    ...
    function docComDetails() {
        ...
        if (!empty($date_from)) {
            ...
            $data['newpatients'] = $this->finance_model->getNewPatientsPaymentByDoctorDate($doctor, $date_from, $date_to);

            $data['execpatients'] = $this->finance_model->getExecutivePatientsDoctorDate($doctor, $date_from, $date_to);
            $data['freeconsult'] = $this->finance_model->getFreeConsultationDoctorDate($doctor, $date_from, $date_to);


        } else {
            $data['payments'] = $this->finance_model->getPaymentByDoctor($doctor);
            $data['newpatients'] = $this->finance_model->getNewPatientsPaymentByDoctor($doctor);
            $data['execpatients'] = $this->finance_model->getExecutivePatientsDoctor($doctor);
            $data['freeconsult'] = $this->finance_model->getFreeConsultationDoctor($doctor);

            // var_dump($data['execpatients']); exit();
        }
